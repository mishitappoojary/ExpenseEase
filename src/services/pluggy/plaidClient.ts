...
export class PlaidClient extends BaseApi {
  ...
  async fetchAccounts() {
    const response = await this.client.get<{ accounts: Account[] }>(
      '/api/plaid/accounts/'
    );
  ...
  async fetchInvestments() {
    ...
    return this.postRequest<{ investments: Investment[] }>(
  ...
  async fetchLiabilities() {
    this.ensureAccessToken();
    return this.postRequest<{ liabilities: Liabilities }>(
      '/liabilities/get',
      {},
    );
  }
  ...
  private ensureAccessToken() {
    if (!this.accessToken) {
      throw new Error(
        'Access token is not set. Please exchange a public token first.',
      );
    }
  }
}
